...
To retrieve a dataset, we can use the `TENxVisiumData()` function, which takes as input one of the dataset identifiers given above. E.g.:
...
Each dataset is provided as a `r Biocpkg("SpatialExperiment")` (SPE), which extends the `r Biocpkg("SingleCellExperiment")` class with features specific to spatially resolved data.
...
```{r}
spe <- TENxVisiumData("MouseBrainSagittalAnterior")
...
```{r}
spe <- TENxVisiumData("HumanOvarianCancer")
altExpNames(spe)
```
